using System;
using System.Collections.Generic;
using System.IO;
using OpenQA.Selenium.Chrome;

internal class Base
{
	public static string txtPathMaxCare = GettxtPathMaxCare();

	private static string string_0 = null;

	private static string CB081526 = null;

	public static Random rd = new Random();

	public static List<string> list_0 = new List<string> { "app.minsoftware.vn", "app.minsoftware.xyz", "beta.minsoftware.vn" };

	public static string B03CF4B4 = Common.Base64Decode("");

	public static string string_1 = Common.Base64Decode("");
    
	public static string jsonAvatar = "";

    public static bool bool_0 = false;

	public static GClass13 E5BBA695;

	public static GClass14 gclass14_0;

	public static ChromeDriverService B20D7004;

	public static string GettxtPathMaxCare()
	{
		if (SettingsTool.GetSettings("configGeneral").GetValueBool("ckbDongBoMaxCare") && Directory.Exists(SettingsTool.GetSettings("configGeneral").GetValue("txtPathMaxCare")))
		{
			return SettingsTool.GetSettings("configGeneral").GetValue("txtPathMaxCare");
		}
		return FileHelper.GetPathToCurrentFolder();
	}

	public static string smethod_1()
	{
		return SettingsTool.GetSettings("settingLogin").GetValue("Server", "1");
	}

	public static Dictionary<string, string> B33DDBA5()
	{
		return new Dictionary<string, string>
		{
			{ "1", "VietNam" },
			{ "2", "English" },
			{ "3", "Cambodia" },
			{ "4", "Thailand" }
		};
	}

	public static string smethod_2()
	{
		string text = smethod_1();
		string text2 = text;
		if (!(text2 == "3"))
		{
			if (!(text2 == "4"))
			{
				string_0 = "https://app.minsoftware.vn";
			}
			else
			{
				string_0 = "https://app.minsoftthailand.com";
			}
		}
		else
		{
			string_0 = "https://app.minsoftcambodia.com";
		}
		return string_0;
	}

	public static string BC13B2B7()
	{
		switch (smethod_1())
		{
		default:
			CB081526 = "11";
			break;
		case "3":
		case "4":
			CB081526 = "1";
			break;
		case "2":
			CB081526 = "61";
			break;
		}
		return CB081526;
	}

	public static string GetLanguage()
	{
		string text = SettingsTool.GetSettings("configGeneral").GetValue("Language");
		if (text == "")
		{
			text = ((!(smethod_1() == "1")) ? "EN" : "VN");
		}
		return text;
	}

	public static void SetLanguage(string language = "VN")
	{
		SettingsTool.GetSettings("configGeneral").GetJson("Language", language);
		SettingsTool.Save("configGeneral");
	}

	public static bool smethod_3()
	{
		return true;
	}
}
