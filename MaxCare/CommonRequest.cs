...
internal class CommonRequest
{
    ...
    public class GraphQLFacebook
    {
        ...
        public InfoUID ResultInfo = default;

        private string uid { get; set; }

        private int row { get; set; }

        public GraphQLFacebook(string uid, int row)
        {
            this.uid = uid;
            this.row = row;
            httpClient.DefaultRequestHeaders.Add("User-Agent", SetupFolder.GetUseragentIphone());
            httpClient.DefaultRequestHeaders.Add("Cookie", "locale=vi_VN");
        ...
        public async Task GetName()
        {
            ...
            string result = (await httpClient.PostAsync("https://www.facebook.com/api/graphql/", content)).Content.ReadAsStringAsync().Result;
            dynamic json = JsonConvert.DeserializeObject(result.Replace(uid, "info"));
    ...
    public static List<string> GetMyListUidNameFriend(string cookie, string token, string userAgent, string proxy, int typeProxy)
    {
        ...
        try
        {
            string value = Regex.Match(cookie + ";", "c_user=(.*?);").Groups[1].Value;
            RequestXNet requestXNet = new RequestXNet(cookie, userAgent, proxy, typeProxy);
            requestXNet.httpRequest.AddHeader("Authorization", "OAuth " + token);
        ...
        try
        {
            ...
            if (isBackupNangCao)
            {
                try
                {
                    ...
                    requestXNet2.httpRequest.AddHeader("Authorization", "OAuth " + token);
    ...
    public static bool CheckAvatarFromUid(string uid, string filePath = "mau.jpg")
    {
        ...
        try
        {
            ...
            double num = hash.Zip(hash2, (i, j) => i == j).Count((eq) => eq) / 256;
    ...
    {
        RequestXNet requestXNet = new RequestXNet("", SetupFolder.GetUseragentIphone(), "", 0);
        string text = "";
        try
        {
    ...
    public static string GetTokenEAAG(string cookie, string userAgent, string proxy, int typeProxy)
    {
        ...
        try
        {
            ...
            string input = requestXNet.RequestGet("https://business.facebook.com/content_management/PUBLISHED_POSTS?global_scope_id=976308133290191&business_id=976308133290191");
    ...
    public static string GetInfoAccountFromUidUsingToken(string tokenTrungGian, string uid, string useragent, string proxy, int typeProxy)
    {
        ...
        try
        {
            ...
            json = requestXNet.RequestGet("https://graph.facebook.com/v18.0/" + uid + "/groups?fields=id&limit=5000&access_token=" + tokenTrungGian);
            jObject = JObject.Parse(json);
            try
            {
                text7 = jObject["data"].Count().ToString() ?? "";
            }
            catch
            {
            }
        ...
        return $"{flag}|{text}|{text2}|{text3}|{text4}|{text5}|{text6}|{text7}";
    }
    ...
    public static string GetInfoAccountFromUidUsingCookie(string cookie, string useragent, string proxy, int typeProxy)
    {
        ...
        try
        {
            ...
            text8 = Regex.Match(input, "EAAA(.*?)\"").Value.TrimEnd('"', '\\');
            text = Regex.Match(input, Common.Base64Decode("cHJvZnBpY1xcIiBhcmlhLWxhYmVsPVxcIiguKj8pLA==")).Groups[1].Value;
            text = WebUtility.HtmlDecode(text);
            string text10 = Common.Base64Decode("");
            text10 = text10.Replace("{{fb_dtsg}}", value2);
            text10 = text10.Replace("{{uid}}", value);
            input = requestXNet.RequestPost("https://www.facebook.com/api/graphql/", text10, "multipart/form-data; boundary=----WebKitFormBoundary2yg0EzDpSZOCXgBR");
            JObject jObject = JObject.Parse(input);
        ...
        return $"{flag}|{text}|{text2}|{text3}|{text4}|{text5}|{text6}|{text7}|{text8}|{text9}";
    }
    ...
    public static string GetInfoAccountFromUidUsingCookieTrungGian(string uid, string cookie_ori)
    {
        ...
        try
        {
            string text10 = "c_user=" + Regex.Match(cookie_ori + ";", "c_user=(.*?);").Groups[1].Value + "; xs=xs=" + Regex.Match(cookie_ori + ";", "xs=(.*?);").Groups[1].Value + ";";
            RequestXNet requestXNet = new RequestXNet(text10 + " useragent=TW96aWxsYS81LjAgKFdpbmRvd3MgTlQgMTAuMDsgV2luNjQ7IHg2NCkgQXBwbGVXZWJLaXQvNTM3LjM2IChLSFRNTCwgbGlrZSBHZWNrbykgQ2hyb21lLzc0LjAuMjMwMi42MSBTYWZhcmkvNTM3LjM2", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:50.0) Gecko/20100101 Firefox/50.0", "", 0);
            ...
            JObject jObject = JObject.Parse(json);
            text6 = jObject[uid]!["friends"]!["count"]!.ToString();
            text7 = jObject[uid]!["groups"]!["count"]!.ToString();
            text = jObject[uid]!["name"]!.ToString();
            text3 = jObject[uid]!["birthday"] != null ? jObject[uid]!["birthday"]!.ToString() : "";
            text2 = jObject[uid]!["gender"]!.ToString().ToLower();
            if (text6 == "")
            {
                text6 = "0";
            }
